(() => {
  ...
  form.addEventListener('submit', async (event) => {
    ...
    data.append('make', makeSelect.value === 'Otra marca' && otherMakeInput ? otherMakeInput.value.trim() : form.elements.make.value.trim());
    data.append('model', form.elements.model.value.trim());
    data.append('year', form.elements.year.value.trim());
    data.append('color', colorSelect.value === 'Otro color' && otherColorInput ? otherColorInput.value.trim() : form.elements.color.value.trim());
    data.append('message', form.elements.message.value.trim());

    serviceInputs.filter((input) => input.checked).forEach((input) => {
    ...
    try {
      const response = await fetch('/send-quote.aspx', {
        method: 'POST',
        body: data
      });
