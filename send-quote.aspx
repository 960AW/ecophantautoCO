...
<script runat="server">
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                var fromAddress = new MailAddress("felipe@960agenciaweb.com", "Ecophant Auto Web");
                var toAddress = new MailAddress("info@ecophantauto.com.co", "Ecophant Auto");
                var bccAddress = new MailAddress("jose@960agenciaweb.com", "Jose 960 Agencia Web");
        ...
        {
            string error = ex.Message ?? string.Empty;
            string userMessage = "No fue posible enviar la solicitud en este momento.";

            if (error.IndexOf("timed out", StringComparison.OrdinalIgnoreCase) >= 0 ||
                error.IndexOf("time-out", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                userMessage = "El servidor de correo no respondio a tiempo. Intenta de nuevo en unos minutos.";
            }
            else if (error.IndexOf("authentication", StringComparison.OrdinalIgnoreCase) >= 0 ||
                     error.IndexOf("credential", StringComparison.OrdinalIgnoreCase) >= 0 ||
                     error.IndexOf("login", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                userMessage = "La cuenta de correo no pudo autenticarse con el servidor SMTP.";
            }
            else if (error.IndexOf("secure connection", StringComparison.OrdinalIgnoreCase) >= 0 ||
                     error.IndexOf("SSL", StringComparison.OrdinalIgnoreCase) >= 0 ||
                     error.IndexOf("TLS", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                userMessage = "No fue posible establecer una conexion segura con el servidor de correo.";
            }

            WriteJson(false, userMessage, 500);
        }
    }
    ...
    private void SendWithFallback(MailMessage mail)
    {
        using (var smtp = new SmtpClient("host.960servidor.com", 465))
        {
            ...
            smtp.Timeout = 15000;
            smtp.Credentials = new NetworkCredential("felipe@960agenciaweb.com", "Hector20-25*.*");
            smtp.Send(mail);
        }
    }
</script>
